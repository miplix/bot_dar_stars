...
Ответ должен быть глубоким, структурированным и практичным."""
    ...
    # ============= КОМПЛЕКСНЫЙ АНАЛИЗ ВСЕХ ДАРОВ =============
    
    async def get_complete_profile_interpretation(self, profile_data: dict) -> str:
        """
        Комплексный анализ всех четырех компонентов (Ода, Туна, Триа, Чиа)
        
        Args:
            profile_data: Словарь с расчетами всех даров
            ...
            async with aiohttp.ClientSession() as session:
                ...
                data = {
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": """Ты эксперт по древнеславянской системе даров рождения Ма-Жи-Кун. 

Твоя задача - дать комплексный анализ человека на основе ЧЕТЫРЕХ расчетов:
1. ОДА (основные данные) - дата рождения - главное влияние
2. ТУНА (второстепенные) - время рождения - меньшее влияние
3. ТРИА (третьестепенные) - место рождения - еще меньшее влияние  
4. ЧИА (четверостепенные) - имя и фамилия - самое малое, но важное влияние

СТРУКТУРА ОТВЕТА (ОБЯЗАТЕЛЬНО):

1. СНАЧАЛА дай краткую суть в 1-3 слова - КТО ЕСТЬ ЭТОТ ЧЕЛОВЕК (например: "Духовный учитель", "Творец-новатор", "Мудрый целитель")

2. Затем раздели описание на разделы:
...
ВАЖНО! Используй ТОЛЬКО Telegram-форматирование:
...
Опирайся на описания из базы данных, но добавляй интерпретацию как они взаимодействуют друг с другом."""
                        },
                        {
                            "role": "user",
                            "content": prompt
    ...
    def _build_complete_prompt(self, profile_data: dict) -> str:
        """Построение промпта для комплексного анализа"""
        oda = profile_data.get('oda', {})
        tuna = profile_data.get('tuna', {})
        tria = profile_data.get('tria', {})
        chia = profile_data.get('chia', {})
        
        prompt = f"""Проанализируй полный профиль человека:

👤 *Имя:* {profile_data.get('name', {}).get('first', '')} {profile_data.get('name', {}).get('last', '')}
📅 *Дата рождения:* {profile_data.get('birth_date', '')}
🕐 *Время рождения:* {profile_data.get('birth_time', '')}
🌍 *Место рождения:* {profile_data.get('location', {}).get('latitude', '')}, {profile_data.get('location', {}).get('longitude', '')}

---

🎁 *ОДА (Основные данные - главное влияние)*
Код: {oda.get('gift_code', '')}
Ма: {oda.get('ma', 0)}, Жи: {oda.get('ji', 0)}, Кун: {oda.get('kun', 0)}
"""
        
        # Добавляем информацию о даре Ода из базы
        oda_gift = get_gift_info(oda.get('gift_code', ''))
        if oda_gift:
            prompt += f"Название: {oda_gift.get('name', '')}\n"
            prompt += f"Описание: {oda_gift.get('description', '')}\n"
        
        prompt += f"""
---

🌙 *ТУНА (Второстепенные данные - меньшее влияние)*
Код: {tuna.get('gift_code', '')}
Ма: {tuna.get('ma', 0)}, Жи: {tuna.get('ji', 0)}, Кун: {tuna.get('kun', 0)}
"""
        
        tuna_gift = get_gift_info(tuna.get('gift_code', ''))
        if tuna_gift:
            prompt += f"Название: {tuna_gift.get('name', '')}\n"
            prompt += f"Описание: {tuna_gift.get('description', '')}\n"
        
        prompt += f"""
---

🌍 *ТРИА (Третьестепенные - влияние места)*
Код: {tria.get('gift_code', '')}
Ма: {tria.get('ma', 0)}, Жи: {tria.get('ji', 0)}, Кун: {tria.get('kun', 0)}
"""
        
        tria_gift = get_gift_info(tria.get('gift_code', ''))
        if tria_gift:
            prompt += f"Название: {tria_gift.get('name', '')}\n"
            prompt += f"Описание: {tria_gift.get('description', '')}\n"
        
        prompt += f"""
---

💫 *ЧИА (Четверостепенные - влияние имени)*
Код: {chia.get('gift_code', '')}
Ма: {chia.get('ma', 0)}, Жи: {chia.get('ji', 0)}, Кун: {chia.get('kun', 0)}
"""
        
        chia_gift = get_gift_info(chia.get('gift_code', ''))
        if chia_gift:
            prompt += f"Название: {chia_gift.get('name', '')}\n"
            prompt += f"Описание: {chia_gift.get('description', '')}\n"
        
        prompt += """
---

Дай комплексный анализ:
1. СНАЧАЛА - Краткое описание в 1-3 слова: КТО этот человек
2. Затем подробный анализ каждого аспекта
3. Как все аспекты взаимодействуют между собой
4. Общую картину личности
5. Рекомендации

Помни про приоритеты влияния: Ода > Туна > Триа > Чиа"""
        
        return prompt
    
    def _get_basic_complete_interpretation(self, profile_data: dict) -> str:
        """Базовая комплексная трактовка без ИИ"""
        oda = profile_data.get('oda', {})
        tuna = profile_data.get('tuna', {})
        tria = profile_data.get('tria', {})
        chia = profile_data.get('chia', {})
        ...
        result = f"🔮 *Комплексный анализ даров*\n\n"
        result += f"👤 *Имя:* {full_name}\n"
        result += f"📅 *Дата:* {profile_data.get('birth_date', '')}\n"
        result += f"🕐 *Время:* {profile_data.get('birth_time', '')}\n\n"
        result += "━━━━━━━━━━━━━━━━━━\n\n"
        
        # ОДА
        result += "🎁 *ОДА - Основа личности*\n"
        result += f"Код: `{oda.get('gift_code', '')}`\n\n"
        oda_gift = get_gift_info(oda.get('gift_code', ''))
        if oda_gift:
            result += f"✨ *{oda_gift.get('name', '')}*\n"
            result += f"{oda_gift.get('description', '')}\n\n"
        
        # ТУНА
        result += "🌙 *ТУНА - Временной аспект*\n"
        result += f"Код: `{tuna.get('gift_code', '')}`\n\n"
        tuna_gift = get_gift_info(tuna.get('gift_code', ''))
        if tuna_gift:
            result += f"✨ *{tuna_gift.get('name', '')}*\n"
            result += f"{tuna_gift.get('description', '')[:200]}...\n\n"
        
        # ТРИА
        result += "🌍 *ТРИА - Энергия места*\n"
        result += f"Код: `{tria.get('gift_code', '')}`\n\n"
        tria_gift = get_gift_info(tria.get('gift_code', ''))
        if tria_gift:
            result += f"✨ *{tria_gift.get('name', '')}*\n"
            result += f"{tria_gift.get('description', '')[:150]}...\n\n"
        
        # ЧИА
        result += "💫 *ЧИА - Имя и судьба*\n"
        result += f"Код: `{chia.get('gift_code', '')}`\n\n"
        chia_gift = get_gift_info(chia.get('gift_code', ''))
        if chia_gift:
            result += f"✨ *{chia_gift.get('name', '')}*\n"
            result += f"{chia_gift.get('description', '')[:150]}...\n\n"
        
        result += "\n💡 _Для получения полного анализа с ИИ настройте API ключ DeepSeek._"
    ...
    async def analyze_mantra(self, mantra_data: dict) -> str:
        """
        Анализ сантры через ИИ на основе данных из баз команд и даров
        
        Args:
            mantra_data: Словарь с данными о сантре (из parse_mantra или create_mantra)
...
Твоя задача - проанализировать предоставленные данные сантры (элементы и дары) и строго сформировать ответ в следующей структуре:

*[Название сантры жирным шрифтом без слова "Название:"]* - 2-3 слова, передающие суть.

*Смысл:* кратко, о чём эта сантра как инструкция для внимания в теле.

*Разбор:* объясни роль и ощущение от каждого ключевого элемента в контексте целого.

*Течение внимания:* опиши пошагово, как внимание должно двигаться по телу, используя «начиная с...», «затем...», «позволяя...» (без нумерованных шагов).

*Практика:* дай одно простое и безопасное упражнение для непосредственного чувствования этой сантры в теле здесь и сейчас.
...
- Опирайся на значения элементов из базы данных, но фокусируйся на телесном опыте"""
                        },
                        {
                            "role": "user",
                            "content": prompt
    ...
    def _build_mantra_prompt(self, mantra_data: dict) -> str:
        """Построение промпта для анализа сантры"""
        mantra_text = mantra_data.get("mantra", "")
        elements = mantra_data.get("elements", [])
        
        prompt = f"""Проанализируй сантру: *{mantra_text}*
...
Проанализируй предоставленные данные сантры (элементы и дары) и строго сформируй ответ в следующей структуре:
...
Будь лаконичным, точным и ориентированным на телесное восприятие."""
    ...
    def _get_basic_mantra_interpretation(self, mantra_data: dict) -> str:
        """Базовая трактовка сантры без ИИ"""
        mantra_text = mantra_data.get("mantra", "")
        elements = mantra_data.get("elements", [])
        errors = mantra_data.get("errors", [])
        
        result = f"🔮 *Анализ сантры*\n\n"
        result += f"📿 *Сантра:* `{mantra_text}`\n\n"
        result += "━━━━━━━━━━━━━━━━━━\n\n"
        
        # Показываем элементы
        result += "*Элементы сантры:*\n\n"
        ...
        for i, elem in enumerate(elements, 1):
            ...
            if elem_type == "команда":
                position = elem.get("position", "")
                result += f"{i}. ⚡ *Команда: {elem_name}*\n"
                result += f"   Позиция: _{position}_\n"
                result += f"   {elem_desc}\n\n"
            elif elem_type == "дар":
                code = elem.get("code", "")
                ma_ji_kun = elem.get("ma_ji_kun", "")
                result += f"{i}. ✨ *Дар: {elem_name}*\n"
                if code:
                    result += f"   Код: `{code}`\n"
                if ma_ji_kun:
                    result += f"   {ma_ji_kun}\n"
                ...
                result += f"{i}. ❓ *Неизвестный элемент: {elem_name}*\n\n"
    ...
    async def get_response(self, prompt: str, user_id: int = None) -> str:
        ...
        """
        if not self.api_key:
            return "⚠️ API ключ DeepSeek не настроен. Пожалуйста, настройте его в файле .env"
            ...
            async with aiohttp.ClientSession() as session:
                ...
                data = {
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": """Ты эксперт по древнеславянской системе даров, алфавиту и эзотерическим знаниям. 
...
Отвечай глубоко, мистически, но лаконично. Только на русском языке."""
                        },
                        {
                            "role": "user",
                            "content": prompt
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        return f"⚠️ Ошибка API ({response.status}): {error_text}"
