...
Использование: 
  python scripts/setup_webhook.py <YOUR_WEBHOOK_URL>
  python scripts/setup_webhook.py <YOUR_WEBHOOK_URL> <BOT_TOKEN>
"""
import sys
import os
import requests
...
def set_webhook(webhook_url: str, bot_token: str = None):
    ...
    if not bot_token:
        ...
        print("  2. Как второй аргумент: python scripts/setup_webhook.py <URL> <TOKEN>")
...
def delete_webhook(bot_token: str = None):
    ...
    if not bot_token:
        ...
        print("\nУстановите токен одним из способов:")
        print("  1. Переменная окружения: set BOT_TOKEN=your_token (Windows) или export BOT_TOKEN=your_token (Linux/Mac)")
        print("  2. Как второй аргумент: python scripts/setup_webhook.py delete <TOKEN>")
        return False
...
def get_webhook_info(bot_token: str = None):
    ...
    if not bot_token:
        ...
        print("\nУстановите токен одним из способов:")
        print("  1. Переменная окружения: set BOT_TOKEN=your_token (Windows) или export BOT_TOKEN=your_token (Linux/Mac)")
        print("  2. Как второй аргумент: python scripts/setup_webhook.py info <TOKEN>")
        return
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Использование:")
        print("  python scripts/setup_webhook.py <webhook_url> [BOT_TOKEN]  - установить webhook")
        print("  python scripts/setup_webhook.py delete [BOT_TOKEN]         - удалить webhook")
        print("  python scripts/setup_webhook.py info [BOT_TOKEN]           - информация о webhook")
        print("\nПример:")
        print("  python scripts/setup_webhook.py https://your-app.vercel.app/api/webhook")
        print("  python scripts/setup_webhook.py https://your-app.vercel.app/api/webhook YOUR_BOT_TOKEN")
